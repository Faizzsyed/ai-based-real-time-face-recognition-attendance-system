...
def build_camera_panel(state,tokens,on_start,on_stop,title="Live Camera",on_retry=None):
    state.retry_button.on_click=on_retry
    state.sync();guide=ft.Container(width=205,height=275,border=ft.Border.all(2,"#94A3B8"),border_radius=999,opacity=.75);preview=ft.Container(content=ft.Stack([state.placeholder,state.image,guide,_corner(left=18,top=18),_corner(right=18,top=18),_corner(left=18,bottom=18),_corner(right=18,bottom=18)],alignment=ft.Alignment.CENTER),height=405,alignment=ft.Alignment.CENTER,bgcolor="#07111F",border_radius=20,border=ft.Border.all(1,"#334155"),clip_behavior=ft.ClipBehavior.ANTI_ALIAS)
    ...
    return surface_card(ft.Column([header,preview,state.status_text,instruction,ft.Row([ft.FilledButton("Start Camera",icon=ft.Icons.VIDEOCAM_OUTLINED,on_click=on_start),state.retry_button,ft.OutlinedButton("Stop Camera",icon=ft.Icons.STOP_CIRCLE_OUTLINED,on_click=on_stop)],alignment=ft.MainAxisAlignment.CENTER,wrap=True)],spacing=14),tokens,key="live-camera-panel")
